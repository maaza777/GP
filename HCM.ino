...
String status = ""; // to print out the status of the sensors if connected or not (SUCCEED | FAILED)
...
PulseOximeter pox;
uint32_t tsLastReport = 0;// for save the value of the time then refreshing the page 
OneWire oneWire(DS18B20);
DallasTemperature sensors(&oneWire);// 22 scl ,21 sda 
...
void setup() {
  Serial.begin(115200); // serial for communication based on the board 
  dht.begin();
  pinMode(19, OUTPUT);
  delay(100);   
  ...
  while (WiFi.status() != WL_CONNECTED) {// return 3 
  delay(1000);
  Serial.print(".");
  }
  Serial.println("");
...
/////db
  ...
  payload = "";
  http.begin("http://172.20.10.11/GP_Project_Final/getData.php");  //--> Specify request destination
...
///db
  ...
  // checking to print the status of the output if success or failed
...
/////db
  ...
  postData += "&BodyTemperature=" + String(bodytemperature);
  postData += "&temperature=" + String(temperature);
  postData += "&humidity=" + String(humidity);
  postData += "&status_=" + status;
  payload = "";



  http.begin("http://172.20.10.11/GP_Project_Final/Updaateesp32_tableRecord.php");  //--> Specify request destination
  http.addHeader("Content-Type", "application/x-www-form-urlencoded");  //--> Specify content-type header
  httpCode = http.POST(postData); //--> Send the request
  payload = http.getString();  //--> Get the response payload
  Serial.print("httpCode : ");
